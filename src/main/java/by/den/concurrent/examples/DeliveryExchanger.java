 ...
 */
public class DeliveryExchanger {
    //Создаем обменник, который будет обмениваться типом String
    private static final Exchanger<String> EXCHANGER = new Exchanger<>();

    public static void main(String[] args) throws InterruptedException {
        String[] p1 = new String[]{"{посылка_t1 A->D}", "{посылка_t1 A->C}"};//Формируем груз для 1-го грузовика
        String[] p2 = new String[]{"{посылка_t2 B->C}", "{посылка_t2 B->D}"};//Формируем груз для 2-го грузовика
    ...
    public static class Truck implements Runnable {
        ...
        public void run() {
            try {
                ...
                System.out.println("Truck "+number+" ждёт ОБМЕНа...");
